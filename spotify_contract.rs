...
declare_id!("Fgd9LxCwwMUuTtS8RSDujZfCBt58QnAhABSp85KRAZCv");
    ...
    pub fn accept_payment(ctx: Context<PayerContext>) -> ProgramResult {
        ...
        anchor_lang::solana_program::program::invoke(
            ...
            &[
                ...
                ctx.accounts.receiever.to_account_info(),
            ],
        )
    }

    pub fn create_music(ctx: Context<CreateMusic>, title: String, music_url: String) -> ProgramResult {
...
pub struct PayerContext<'info> {
    #[account(init,seeds = [b"payer".as_ref(), authority.key().as_ref()],
              bump,
              payer = authority, space = size_of::<PayerAccount>() + 8)]
    pub payer_wallet: Account<'info, PayerAccount>,
    #[account(mut)]
    pub receiever: AccountInfo<'info>,
...
pub struct CreateMusic<'info> {
    #[account(init, seeds=[b"music".as_ref() , randomkey.key().as_ref()],
             bump,
             payer = authority,
